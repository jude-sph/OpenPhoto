...
### `people` (v8, rebuildable mirror of XMP sidecars; `coverFaceID` added v12)
...
| `coverFaceID` | INTEGER? | Nullable. References `faces.id`. The user-chosen cover face displayed on the People screen ("Use as Thumbnail"). A Mac-local display preference — snapshot readers MAY ignore it entirely. Stale values (face deleted or reassigned to another person) are never cleaned up eagerly; `people()` silently falls back to the highest-confidence face via `COALESCE` when the stored `faceID` no longer belongs to this person. Added in schema v12; readers MUST NOT rely on this column being present in snapshots below v12. |
