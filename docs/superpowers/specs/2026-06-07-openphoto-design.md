...
3. **Drives** ⬅ in progress — canonical sync (**Slice 1 ✅**: additive Mac→canonical sync spine), evict/rehydrate, clone/migrate, drift review, integrity verify, delete propagation, catalog snapshot. *Decomposed into slices; see the 2026-06-09 changelog entry and `docs/superpowers/specs/2026-06-09-phase3-drives-design.md`.* Slices done: 1 (sync), 2 (drift/integrity), 3 (deletion propagation), 4 (evict/rehydrate), 4.5 (send-from-drive), 5a (clone + durable backups), 5b (catalog snapshot + adoption). Remaining: **5c** (canonical management & migration), then merge → `main`; **5d** below is optional/after.
   - **Slice 5d (planned, optional — after 5c + the phase-3 merge): Quick View (ephemeral drive peek).** Plug a drive in and **browse it without adopting** — a sandboxed, trace-free peek; on **Done**/unplug nothing persists ("like it was never here"). Design (from the 2026-06-10 discussion): never mix peeked photos into the main timeline (confusing) — open a separate **"Viewing '<Drive>'"** context (its own grid + the normal full-screen viewer), clearly *not your library*, with a **Done** button; reuses the **Import-grid** pattern ("browse an external source's photos without importing"). Trace-free data: an OpenPhoto drive with a `catalog-snapshot/` is read into a **throwaway catalog in a temp dir** (thumbnails from the snapshot, full-res from the plugged-in drive via the Slice-5a `driveSource` resolution); a drive/folder *without* a snapshot is lazily scanned + thumbnailed as you scroll (like Import previews an SD card) into a temp cache — **nothing is written to the live catalog or thumbnail cache**, and the temp dir is deleted on close. Entry point: a **Quick View** option alongside *Adopt / Not now* on the unknown-drive prompt (and/or a drive-row button). Build the **lightweight self-contained peek window first** (a grid + viewer reading the temp catalog, not wired into the sidebar) — avoids re-architecting the main browse layer to be multi-source; a "parameterize browse by source" version is a possible later refinement. A UI investment, not a hard data problem; off the Phase 3 critical path.
